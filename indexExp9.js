...
app.post('/api/item', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    console.log('AWS S3 Upload Result:', { err, data });

    if (err) {
      console.error('Error uploading image:', err);
      return res.status(500).json({ error: 'Failed to upload image to S3', details: err });
    }

    // Check if the 'data' object is undefined or does not have the 'Location' property
    if (!data || typeof data !== 'object' || !data.hasOwnProperty('Location')) {
      console.error('Invalid S3 upload response:', data);
      return res.status(500).json({ error: 'Invalid S3 upload response' });
    }
    ...
    res.status(500).json({ error: 'Failed to save the item', details: error.message });
  }
});
*/
